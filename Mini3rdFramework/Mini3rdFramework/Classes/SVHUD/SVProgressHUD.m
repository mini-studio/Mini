...
@synthesize overlayWindow, hudView, maskType, showNetworkIndicator, fadeOutTimer, stringLabel, imageView, spinnerView, visibleKeyboardHeight, timeOutTimer;
...
- (void)dealloc {
    ...
    if ( timeoutblock != nil )
    {
        Block_release( timeoutblock );
    }
    [super dealloc];
}

- (void)setTimeoutBlock:(TimeoutBlock)theTimeoutblock
{
    timeoutblock = Block_copy(theTimeoutblock); 
...
{
    self.timeOutTimer = nil;
    [SVProgressHUD dismissWithError:@"发生超时"];
    timeoutblock();
}
...
{
    [SVProgressHUD showWithStatus:status networkIndicator:SVProgressHUDShowNetworkIndicator];
    [[SVProgressHUD sharedView] setTimeoutBlock:block];
}
...
+ (void)showWithStatus:(NSString *)status networkIndicator:(BOOL)show {
    [SVProgressHUD showWithStatus:status maskType:SVProgressHUDMaskTypeClear networkIndicator:show];
}
	...
	[UIView animateWithDuration:0.15
					 ...
					 completion:^(BOOL finished){ 
                         if(sharedView.alpha == 0) {
                             ...
                             //LOG_DEBUG(@"%@", [UIApplication sharedApplication].windows);
                             //LOG_DEBUG(@"keyWindow = %@", [UIApplication sharedApplication].keyWindow);
                         }
                     }];
}
