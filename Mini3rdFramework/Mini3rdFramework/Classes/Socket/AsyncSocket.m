...
- (BOOL)acceptOnInterface:(NSString *)interface port:(UInt16)port error:(NSError **)errPtr
{
	...
	if (theSocket4)
	{
		...
		//LOG_DEBUG(@"theSocket4: %hu", [self localPortFromCFSocket4:theSocket4]);
	}
	...
	if (theSocket6)
	{
		...
		//LOG_DEBUG(@"theSocket6: %hu", [self localPortFromCFSocket6:theSocket6]);
	}
...
- (BOOL)createStreamsFromNative:(CFSocketNativeHandle)native error:(NSError **)errPtr
{
	...
	if (theReadStream == NULL || theWriteStream == NULL)
	{
		...
		LOG_ERROR(@"AsyncSocket %p couldn't create streams from accepted socket: %@", self, err);
...
- (BOOL)attachStreamsToRunLoop:(NSRunLoop *)runLoop error:(NSError **)errPtr
{
							   ...
							   (CFStreamClientContext *)(&theContext)))
	{
		...
		LOG_ERROR(@"AsyncSocket %p couldn't attach read stream to run-loop,", self);
		LOG_ERROR(@"Error: %@", err);
								 ...
								 (CFStreamClientContext *)(&theContext)))
	{
		...
		LOG_ERROR(@"AsyncSocket %p couldn't attach write stream to run-loop,", self);
		LOG_ERROR(@"Error: %@", err);
...
- (BOOL)openStreamsAndReturnError:(NSError **)errPtr
{
	...
	{
		LOG_ERROR(@"AsyncSocket %p couldn't open read stream,", self);
		pass = NO;
	}
	
	if(pass && !CFWriteStreamOpen(theWriteStream))
	{
		LOG_ERROR(@"AsyncSocket %p couldn't open write stream,", self);
		pass = NO;
	}
...
- (void)doStreamOpen
{
	if ((theFlags & kDidCompleteOpenForRead) && (theFlags & kDidCompleteOpenForWrite))
	{
		...
		{
			LOG_ERROR(@"AsyncSocket %p couldn't get socket from streams, %@. Disconnecting.", self, err);
			[self closeWithError:err];
			return;
		}
...
- (BOOL)setSocketFromStreamsAndReturnError:(NSError **)errPtr
{
	...
	if(peeraddr == NULL)
	{
		LOG_ERROR(@"AsyncSocket couldn't determine IP version of socket");
...
- (NSError *)getSocketError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"Halo.net.AsyncSocketCFSocketError",
														 @"Halo", [NSBundle mainBundle],
...
- (NSError *)getAbortError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"Halo.net.AsyncSocketCanceledError",
														 @"Halo", [NSBundle mainBundle],
...
- (NSError *)getConnectTimeoutError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"Halo.net.AsyncSocketConnectTimeoutError",
														 @"Halo", [NSBundle mainBundle],
...
- (NSError *)getReadMaxedOutError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"Halo.net.AsyncSocketReadMaxedOutError",
														 @"Halo", [NSBundle mainBundle],
...
- (NSError *)getReadTimeoutError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"Halo.net.AsyncSocketReadTimeoutError",
														 @"Halo", [NSBundle mainBundle],
...
- (NSError *)getWriteTimeoutError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"Halo.net.AsyncSocketWriteTimeoutError",
														 @"Halo", [NSBundle mainBundle],
				  ...
				  withData:(const void *)pData
{
	...
	switch (type)
	{
		...
		case kCFSocketAcceptCallBack:
			...
			break;
		default:
			LOG_ERROR(@"AsyncSocket %p received unexpected CFSocketCallBackType %i", self, (int)type);
			break;
	}
}
...
- (void)doCFReadStreamCallback:(CFStreamEventType)type forStream:(CFReadStreamRef)stream
{
	...
	switch (type)
	{
		...
		case kCFStreamEventEndEncountered:
			...
			break;
		default:
			LOG_ERROR(@"AsyncSocket %p received unexpected CFReadStream callback, CFStreamEventType %i", self, (int)type);
	}
}
...
- (void)doCFWriteStreamCallback:(CFStreamEventType)type forStream:(CFWriteStreamRef)stream
{
	...
	switch (type)
	{
		...
		case kCFStreamEventEndEncountered:
			...
			break;
		default:
			LOG_ERROR(@"AsyncSocket %p received unexpected CFWriteStream callback, CFStreamEventType %i", self, (int)type);
	}
}
